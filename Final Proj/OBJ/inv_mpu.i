...
#line 16 "..\\BALANCE\\DMP\\inv_mpu.c"
#line 17 "..\\BALANCE\\DMP\\inv_mpu.c"
#line 18 "..\\BALANCE\\DMP\\inv_mpu.c"
#line 19 "..\\BALANCE\\DMP\\inv_mpu.c"
...
#line 106 "..\\BALANCE\\DMP\\inv_mpu.c"
...
#line 141 "..\\BALANCE\\DMP\\inv_mpu.c"
...
#line 202 "..\\BALANCE\\DMP\\inv_mpu.c"
};
...
#line 278 "..\\BALANCE\\DMP\\inv_mpu.c"
};
...
enum lp_accel_rate_e {
    ...
    INV_LPA_20HZ,
    INV_LPA_40HZ
#line 365 "..\\BALANCE\\DMP\\inv_mpu.c"
};

#line 408 "..\\BALANCE\\DMP\\inv_mpu.c"

#line 416 "..\\BALANCE\\DMP\\inv_mpu.c"

#line 444 "..\\BALANCE\\DMP\\inv_mpu.c"
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            ...
            tmp = 0x00;
        if (i2cWrite(st.hw->addr, st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_reg_dump(void)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++) {
        if (ii == st.reg->fifo_r_w || ii == st.reg->mem_r_w)
            continue;
        if (i2cRead(st.hw->addr, ii, 1, &data))
            return -1;
...
int mpu_read_reg(unsigned char reg, unsigned char *data)
{
    ...
    if (reg >= st.hw->num_reg)
        return -1;
    return i2cRead(st.hw->addr, reg, 1, data);
}
...
int mpu_init(void)
{
    ...
    data[0] = 0x80;
    if (i2cWrite(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(data[0])))
        return -1;
    delay_ms(100);
    ...
    if (i2cRead(st.hw->addr, st.reg->accel_offs, 6, data))
        return -1;
    ...
    if (rev) {
        ...
        if (i2cRead(st.hw->addr, st.reg->prod_id, 1, &(data[0])))
            return -1;
        rev = data[0] & 0x0F;
        if (!rev) {
...
#line 823 "..\\BALANCE\\DMP\\inv_mpu.c"
...
int mpu_lp_accel_mode(unsigned char rate)
{
    ...
    if (!rate) {
        ...
        if (i2cWrite(st.hw->addr, st.reg->pwr_mgmt_1, 2, tmp))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
    ...
    if (i2cWrite(st.hw->addr, st.reg->pwr_mgmt_1, 2, tmp))
        return -1;
#line 960 "..\\BALANCE\\DMP\\inv_mpu.c"
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2cRead(st.hw->addr, st.reg->raw_gyro, 6, tmp))
        return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2cRead(st.hw->addr, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (i2cRead(st.hw->addr, st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp)
        myget_ms(timestamp);
...
int mpu_set_accel_bias(const long *accel_bias)
{
    ...
    if (i2cRead(st.hw->addr, 3, 3, data))
        return -1;
    fg[0] = ((data[0] >> 4) + 8) & 0xf;
    fg[1] = ((data[1] >> 4) + 8) & 0xf;
    fg[2] = ((data[2] >> 4) + 8) & 0xf;
    ...
    if (i2cRead(st.hw->addr, 0x06, 6, data))
        return -1;
    ...
    if (i2cWrite(st.hw->addr, 0x06, 6, data))
        return -1;
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (i2cWrite(st.hw->addr, st.reg->int_enable, 1, &data))
        return -1;
    if (i2cWrite(st.hw->addr, st.reg->fifo_en, 1, &data))
        return -1;
    if (i2cWrite(st.hw->addr, st.reg->user_ctrl, 1, &data))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (i2cWrite(st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
        if (i2cWrite(st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (i2cWrite(st.hw->addr, st.reg->gyro_cfg, 1, &data))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (i2cWrite(st.hw->addr, st.reg->accel_cfg, 1, &data))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (i2cWrite(st.hw->addr, st.reg->lpf, 1, &data))
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        data = 1000 / rate - 1;
        if (i2cWrite(st.hw->addr, st.reg->rate_div, 1, &data))
            return -1;
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
#line 1451 "..\\BALANCE\\DMP\\inv_mpu.c"
    return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & ((0x40) | (0x20) | (0x10)))
        ...
        data = (0x40);
    if (i2cWrite(st.hw->addr, st.reg->pwr_mgmt_1, 1, &data)) {
    ...
    if (!(sensors & (0x08)))
        data |= ((0x20) | (0x10) | (0x08));
    if (i2cWrite(st.hw->addr, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
...
#line 1659 "..\\BALANCE\\DMP\\inv_mpu.c"
...
int mpu_get_int_status(short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (i2cRead(st.hw->addr, st.reg->dmp_int_status, 2, tmp))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (i2cRead(st.hw->addr, st.reg->fifo_count_h, 2, data))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (i2cRead(st.hw->addr, st.reg->int_status, 1, data))
            return -1;
        if (data[0] & (0x10)) {
            mpu_reset_fifo();
            return -2;
    ...
    myget_ms((unsigned long*)timestamp);

    if (i2cRead(st.hw->addr, st.reg->fifo_r_w, packet_size, data))
        return -1;
    ...
    unsigned char *more)
{
    ...
    if (i2cRead(st.hw->addr, st.reg->fifo_count_h, 2, tmp))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (i2cRead(st.hw->addr, st.reg->int_status, 1, tmp))
            return -1;
        if (tmp[0] & (0x10)) {
            mpu_reset_fifo();
            return -2;
    ...
    if (i2cRead(st.hw->addr, st.reg->fifo_r_w, length, data))
        return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (i2cRead(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~(0x20);
        if (i2cWrite(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        delay_ms(3);
        tmp = (0x02);
        ...
        if (st.chip_cfg.latched_int)
            tmp |= (0x20) | (0x10);
        if (i2cWrite(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
    } else {
...
static int get_accel_prod_shift(float *st_shift)
{
    ...
    if (i2cRead(st.hw->addr, 0x0D, 4, tmp))
        return 0x07;
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
    ...
    if (i2cRead(st.hw->addr, 0x0D, 3, tmp))
        return 0x07;
...
#line 2030 "..\\BALANCE\\DMP\\inv_mpu.c"
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (i2cWrite(st.hw->addr, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    delay_ms(200);
    data[0] = 0;
    if (i2cWrite(st.hw->addr, st.reg->int_enable, 1, data))
    ...
    if (i2cWrite(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (i2cWrite(st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
    if (i2cWrite(st.hw->addr, st.reg->user_ctrl, 1, data))
        return -1;
    data[0] = (0x04) | (0x08);
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (i2cRead(st.hw->addr, st.reg->fifo_r_w, (12), data))
            return -1;
...
#line 2132 "..\\BALANCE\\DMP\\inv_mpu.c"
...
restore:
#line 2243 "..\\BALANCE\\DMP\\inv_mpu.c"
        ...
        unsigned char *data)
{
    ...
    if (i2cWrite(st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2cWrite(st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
        ...
        unsigned char *data)
{
    ...
    if (i2cWrite(st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2cRead(st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
    ...
    unsigned short start_addr, unsigned short sample_rate)
{
    ...
    if (i2cWrite(st.hw->addr, st.reg->prgm_start_h, 2, tmp))
        return -1;
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable) {
        ...
        tmp = 0;
        i2cWrite(st.hw->addr, 0x23, 1, &tmp);
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
#line 2581 "..\\BALANCE\\DMP\\inv_mpu.c"
    return -1;
    ...
    unsigned char lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        if (thresh > 8160)
            ...
            thresh_hw = thresh >> 5;
#line 2670 "..\\BALANCE\\DMP\\inv_mpu.c"
        ...
        data[0] = thresh_hw;
        data[1] = time;
        if (i2cWrite(st.hw->addr, st.reg->motion_thr, 2, data))
            goto lp_int_restore;
        ...
        return 0;
#line 2811 "..\\BALANCE\\DMP\\inv_mpu.c"
    } else {
...
#line 2847 "..\\BALANCE\\DMP\\inv_mpu.c"
